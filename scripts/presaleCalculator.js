...
module.exports = (_tokensPerEth) => {
  ...
  const clnTokenUsdPriceSigDig = new BigNumber('95238095238095238095238095238095238');
  ...
  return {
    calcPresale: (presales) => {
      return presales.map((presale) => {
        ...
        let tokensToReceive = new BigNumber(presale.dolarInvest).mul(decimals).mul(decimals).mul(decimals).div(clnTokenUsdPriceSigDig);
