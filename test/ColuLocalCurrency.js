...
contract('ColuLocalCurrency', (accounts) => {
  ...
  describe('construction', () => {
    it('should not construct without a name', async () => {
      await expectRevert(ColuLocalCurrency.new('', 'MC', 18, MAX_TOKENS, 'metadatahash', owner, {from: owner}))
    })

    it('should not construct without a symbol', async () => {
      await expectRevert(ColuLocalCurrency.new('MasterCoin', '', 18, MAX_TOKENS, 'metadatahash', owner, {from: owner}))
    })

    it('should not construct without a totalSupply', async () => {
      await expectRevert(ColuLocalCurrency.new('MasterCoin', 'MC', 18, 0, 'metadatahash', owner, {from: owner}))
    })

    it('should not construct without a issuer', async () => {
    ...
    it('should be able to construct currency with all args', async () => {
      const cc = await ColuLocalCurrency.new('MasterCoin', 'MC', 18, MAX_TOKENS, 'metadatahash', owner, {from: owner})
      ...
      assert.equal((await cc.issuer()), owner)
    })

    it('should be able to construct currency without decimals', async () => {
      const cc = await ColuLocalCurrency.new('MasterCoin', 'MC', 0, MAX_TOKENS, 'metadatahash', owner, {from: owner})
      assert.equal((await cc.decimals()), 0)
    })

    it('should be able to construct currency without metadata', async () => {
      const cc = await ColuLocalCurrency.new('MasterCoin', 'MC', 18, MAX_TOKENS, '', owner, {from: owner})
  ...
  describe('actions', () => {
    beforeEach(async () => {
      cc = await ColuLocalCurrency.new('MasterCoin', 'MC', 0, MAX_TOKENS, 'metadatahash', owner, {from: owner})
    })
