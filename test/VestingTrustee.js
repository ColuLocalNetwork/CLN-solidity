...
contract('VestingTrustee', (accounts) => {
    ...
    describe('grant', async () => {
        ...
        context('using token.transfer', async () => {
            context(`with ${balance} tokens assigned to the trustee`, async () => {
                ...
                it('should not allow granting to 0', async () => {
                    await token.transfer[tokenTransferWithDataSig](trustee.address, 1000, getDataForGrantUsingTransfer(0, now, now, now + 10 * YEAR, 1 * DAY, false));
                    assert.equal((await trustee.totalVesting()).toNumber(), 0);
                });

                it('should not allow granting to self', async () => {
                    await token.transfer[tokenTransferWithDataSig](trustee.address, 1000, getDataForGrantUsingTransfer(trustee.address, now, now, now + 10 * YEAR, 1 * DAY, false));
                    assert.equal((await trustee.totalVesting()).toNumber(), 0);
                });

                it('should not allow granting 0 tokens', async () => {
                    await token.transfer[tokenTransferWithDataSig](trustee.address, 0, getDataForGrantUsingTransfer(accounts[0], now, now, now + 3 * YEAR, 1 * DAY, false));
                    assert.equal((await trustee.totalVesting()).toNumber(), 0);
                });

                it('should not allow granting with a cliff before the start', async () => {
                    await token.transfer[tokenTransferWithDataSig](trustee.address, 0, getDataForGrantUsingTransfer(accounts[0], now, now - 1, now + 10 * YEAR, 1 * DAY, false));
                    assert.equal((await trustee.totalVesting()).toNumber(), 0);
                });

                it('should not allow granting with a cliff after the vesting', async () => {
                    await token.transfer[tokenTransferWithDataSig](trustee.address, 0, getDataForGrantUsingTransfer(accounts[0], now, now + YEAR, now + MONTH, 1 * DAY, false));
                    assert.equal((await trustee.totalVesting()).toNumber(), 0);
                });

                it('should not allow granting with 0 installment', async () => {
                    await token.transfer[tokenTransferWithDataSig](trustee.address, 0, getDataForGrantUsingTransfer(accounts[0], now, now + YEAR, now + MONTH, 0, false));
                    assert.equal((await trustee.totalVesting()).toNumber(), 0);
                });

                it('should not allow granting with installment longer than the vesting period', async () => {
                    await token.transfer[tokenTransferWithDataSig](trustee.address, 0, getDataForGrantUsingTransfer(accounts[0], now, now + YEAR, now + MONTH, 2 * YEAR, false));
                    assert.equal((await trustee.totalVesting()).toNumber(), 0);
                });

                it('should not allow granting tokens more than once', async () => {
                    let data = getDataForGrantUsingTransfer(accounts[0], now, now, now + 10 * YEAR, 1 * DAY, false);

                    await token.transfer[tokenTransferWithDataSig](trustee.address, 1000, data);
                ...
                it('should not allow granting from not an owner', async () => {
                    await token.transfer[tokenTransferWithDataSig](trustee.address, 1000, getDataForGrantUsingTransfer(accounts[0], now, now + MONTH, now + YEAR, 1 * DAY, false, {from: accounts[1]}));
                    assert.equal((await trustee.totalVesting()).toNumber(), 1000);
                });
                ...
                it('should record a grant and increase grants count and total vesting', async () => {
                    ...
                    let end = now + YEAR;
                    let installmentLength = 1 * DAY;
                    await token.transfer[tokenTransferWithDataSig](trustee.address, value, getDataForGrantUsingTransfer(accounts[0], start, cliff, end, installmentLength, false));
                    ...
                    let end2 = now + YEAR;
                    let installmentLength2 = 3 * MONTH;
                    await token.transfer[tokenTransferWithDataSig](trustee.address, value2, getDataForGrantUsingTransfer(accounts[1], start2, cliff2, end2, installmentLength2, false));
        ...
        context('using token.transfer', async () => {
            context(`with ${balance} tokens assigned to the trustee`, async () => {
                ...
                context('after transfering has ended', async () => {
                    ...
                    it('should not be able to revoke a non-revokable grant', async () => {
                        await token.transfer[tokenTransferWithDataSig](trustee.address, balance, getDataForGrantUsingTransfer(grantee, now, now + MONTH, now + YEAR, 1 * DAY, false));
                    ...
                    context(`grant: ${grant.tokens}, startOffset: ${grant.startOffset}, cliffOffset: ${grant.cliffOffset}, ` +
                        ...
                        for (let i = 0; i < grant.results.length; ++i) {
                            it(`should revoke the grant and refund tokens after ${i + 1} transactions`, async () => {
                                ...
                                await token.transfer(trustee.address, grant.tokens);
                                await token.makeTokensTransferable();
                                await token.transfer[tokenTransferWithDataSig](trustee.address, grant.tokens, getDataForGrantUsingTransfer(holder, now + grant.startOffset, now + grant.cliffOffset, now + grant.endOffset, grant.installmentLength, true));
        ...
        grants.forEach((grant) => {
            ...
            context('using token.transfer', async () => {
                context(`grant: ${grant.tokens}, startOffset: ${grant.startOffset}, cliffOffset: ${grant.cliffOffset}, ` +
                    ...
                    beforeEach(async () => {
                        await token.transfer[tokenTransferWithDataSig](trustee.address, grant.tokens, getDataForGrantUsingTransfer(accounts[2], now + grant.startOffset, now + grant.cliffOffset, now + grant.endOffset, grant.installmentLength, false));
                    });
            ...
            grants.forEach(async (grant) => {
                ...
                context('using token.transfer', async () => {
                    context(`grant: ${grant.tokens}, startOffset: ${grant.startOffset}, cliffOffset: ${grant.cliffOffset}, ` +
                        ...
                        beforeEach(async () => {
                            await token.transfer[tokenTransferWithDataSig](trustee.address, grant.tokens, getDataForGrantUsingTransfer(holder, now + grant.startOffset, now + grant.cliffOffset, now + grant.endOffset, grant.installmentLength, false));
                        });
    ...
    describe('events', async () => {
        ...
        context('using token.transfer', async () => {
            ...
            it('should emit events when revoking a grant', async () => {
                await token.transfer[tokenTransferWithDataSig](trustee.address, value, getDataForGrantUsingTransfer(grantee, start, cliff, end, installmentLength, true));
                let result = await trustee.revoke(grantee);
